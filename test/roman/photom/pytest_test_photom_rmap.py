...
class PhotomPyTests:
    def pytest_test_photom_rmap_exists(self):
        ...
        assert os.path.exists(
            Path(__file__).parents[3] / "crds" / "roman" / "specs" / "wfi_photom.rmap"
        )
    ...
    def pytest_test_photom_tpn_exist(self):
        ...
        assert os.path.exists(
            Path(__file__).parents[3] / "crds" / "roman" / "tpns" / "wfi_photom.tpn"
        )
    ...
    def pytest_test_photom_tpn_headers(self):
        ...
        """
        expected_headers = ["ROMAN.META.INSTRUMENT.DETECTOR"]
        with open(
            Path(__file__).parents[3] / "crds" / "roman" / "tpns" / "wfi_photom.tpn",
            "r",
        ) as photom_tpn:
