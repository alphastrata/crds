...
class AreaPyTests:
    def pytest_test_area_rmap_exists(self):
        ...
        assert os.path.exists(
            Path(__file__).parents[3] / "crds" / "roman" / "specs" / "wfi_area.rmap"
        )
    ...
    def pytest_test_area_tpn_exist(self):
        ...
        assert os.path.exists(
            Path(__file__).parents[3] / "crds" / "roman" / "tpns" / "wfi_area.tpn"
        )
    ...
    def pytest_test_area_tpn_headers(self):
        ...
        with open(
            Path(__file__).parents[3] / "crds" / "roman" / "tpns" / "wfi_area.tpn", "r"
        ) as area_tpn:
            lines = [line.replace("\n", "") for line in area_tpn.readlines()]

            headers = [
                s[: s.index(" ")]
                for s in filter(lambda l: l != "" and l[0] != "#", lines)
            ]
