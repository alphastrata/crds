...
def get_affected(old_pmap, new_pmap, *args, **keys):
    ...
    """
    observatory = keys.pop(
        "observatory", crds.get_pickled_mapping(old_pmap).observatory
    )  # reviewed
...
def difference(observatory, old_file, new_file, *args, **keys):
    ...
    differ_class = differs.get(filetype)
    if differ_class is None:
        log.warning(
            "Cannot difference file of type",
            repr(filetype),
            ":",
            repr(old_file),
            repr(new_file),
...
class Differencer:
    ...
    def __init__(
        ...
        self.cache1 = cache1
        self.cache2 = cache2
        self.mappings_cache1 = (
            os.path.join(self.cache1, "mappings", self.observatory) if cache1 else None
        )
        self.mappings_cache2 = (
            os.path.join(self.cache2, "mappings", self.observatory) if cache2 else None
        )
...
class MappingDifferencer(Differencer):
    ...
    def __init__(self, *args, **keys):
        ...
        assert config.is_mapping(
            self.old_file
        ), f"File {repr(self.old_file)} is not a CRDS mapping."

        assert config.is_mapping(
            self.new_file
        ), f"File {repr(self.new_file)} is not a CRDS mapping."

        assert (
            os.path.splitext(self.old_file)[-1] == os.path.splitext(self.new_file)[-1]
        ), (
            "Files "
            + repr(self.old_file)
            + " and "
    ...
    def difference(self):
        ...
        for diff in sorted(differences):
            ...
            if self.primitive_diffs:
                log.write("=" * 80)
            diff2 = (
                simplify_to_lowest_mapping(diff1) if self.lowest_mapping_only else diff1
            )
            ...
            if (
                self.primitive_diffs
                and "header" not in diff_action(diff1)
                and "replaced" in diff[-1]
            ):
        ...
        pairs = sorted(
            set(mapping_pairs(differences) + [(self.old_file, self.new_file)])
        )

        for old, new in pairs:
            if self.mapping_text_diffs:
                log.write(
                    "=" * 20, "text difference", repr(old), "vs.", repr(new), "=" * 20
                )
    ...
    def squash_diff_tuples(self, diff2):
        ...
        return " -- ".join(
            [" ".join(diff) if isinstance(diff, tuple) else diff for diff in diff2]
        )
    ...
    def mapping_diffs(self):
        ...
        """
        old_map = rmap.fetch_mapping(
            self.locate_file1(self.old_file),
            ignore_checksum=True,
            path=self.mappings_cache1,
        )

        new_map = rmap.fetch_mapping(
            self.locate_file2(self.new_file),
            ignore_checksum=True,
            path=self.mappings_cache2,
        )

        return old_map.difference(
            new_map,
            include_header_diffs=self.include_header_diffs,
            recurse_added_deleted=self.recurse_added_deleted,
        )
        ...
        for diff in diffs:
            for step in diff:
                ...
                if (
                    instrument.strip()
                    and filekind.strip()
                    and filekind not in instrs[instrument]
                ):
                    log.verbose(
                        "Affected",
                        (instrument, filekind),
    ...
    def _find_diff_str(self, diff_str):
        ...
        for diff in diffs:
            for step in diff:
                if (
                    len(step) == 2
                    and config.is_mapping(step[0])
                    and diff_str in diff_action(diff)
                ):
                    log.verbose("Found", repr(diff_str), "diff between", repr(step[:1]))
                    return True
        return False
...
class FitsDifferencer(Differencer):
    ...
    def __init__(self, *args, **keys):
        ...
        assert self.old_file.endswith(
            ".fits"
        ), f"File {repr(self.old_file)} is not a FITS file."

        assert self.new_file.endswith(
            ".fits"
        ), f"File {repr(self.new_file)} is not a FITS file."
...
class AsdfDifferencer(Differencer):
    ...
    def diff(self):
        ...
        if not status and len(out_err) != 0:
            status = 1
        out_err = "" if len(out_err) == 0 else decolorize(out_err)
        return status, out_err
...
def simplify_to_lowest_mapping(diff):
    ...
    simplified = [
        ...
        or not tup[0].endswith("map")
        or tup[0] == lowest_ranked_mapping
    ]
...
def unquote_diff(diff):
    ...
    return (
        diff[:-1] + (diff[-1].replace("'", ""),) if "header" not in diff[-1] else diff
    )
...
def diff_replace_old_new(diff):
    ...
    if match := re.search("replaced '(.+)' with '(.+)'", diff[-1]):
...
def diff_action(diff):
    ...
    if "replace" in diff[-1]:
        ...
        raise ValueError(f"Bad difference action: {repr(diff)}")
    if "rule" in diff[-1]:
        result += "_rule"
...
def mapping_affected_modes(old_file, new_file, include_header_diffs=True):
    ...
    differences = mapping_diffs(
        old_file, new_file, include_header_diffs=include_header_diffs
    )
    for diff in differences:
    ...
    for i, val in enumerate(flat):
        ...
        if var == "PipelineContext":
            ...
            for boring in BORING_VARS:
                if f"REPLACED {repr(boring)}" in val.upper():
                    break
            else:
                affected.append((var, val))
    if not affected:
...
def remove_boring(diffs):
    ...
    result = diffs[:]
    for diff, var in itertools.product(diffs, BORING_VARS):
...
def mapping_check_diffs(mapping, derived_from):
    ...
    derived_from = rmap.asmapping(derived_from, cached="readonly")
    log.info(
        "Checking diffs from", repr(derived_from.basename), "to", repr(mapping.basename)
    )
    ...
    for action, msg in categorized:
        if "header" in action:
            ...
            if old_val and new_val:
                old_val, new_val = list(diff_replace_old_new(msg))
...
def get_added_references(old_pmap, new_pmap, cached=True):
    ...
    return sorted(
        list(set(new_pmap.reference_names()) - set(old_pmap.reference_names()))
    )
...
def get_deleted_references(old_pmap, new_pmap, cached=True):
    ...
    new_pmap = rmap.asmapping(new_pmap, cached=cached)
    return sorted(
        list(set(old_pmap.reference_names()) - set(new_pmap.reference_names()))
    )
...
def get_updated_files(context1, context2):
    ...
    all_mappings = rmap.list_mappings(f"*{extension1}", old_map.observatory)
    updated = set()
...
Mutually Exclusive Modes
    ...
    def main(self):
        ...
        self.args.files = [self.args.old_file, self.args.new_file]
        ...
        if self.args.sync_files:
            ...
            if self.args.print_all_new_files:
                ...
                if None not in [serial_old, serial_new]:
                    errs = sync.SyncScript(
                        "crds.sync --range {0}:{1}".format(serial_old, serial_new)
                    )()

                    assert (
                        not errs
                    ), "Errors occurred while syncing all rules to CRDS cache."
                else:
                    log.warning(
        ...
        assert (
            self.args.cache1
            and self.args.cache2
            or not self.args.cache1
            and not self.args.cache2
        ...
        return 2 if log.errors() or log.warnings() else status
    ...
    def print_affected_modes(self):
        ...
        assert config.is_mapping(self.old_file) and config.is_mapping(
            self.new_file
        ), "for --print-affected-modes both files must be mappings."
        modes = mapping_affected_modes(
            self.old_file, self.new_file, self.args.include_header_diffs
        )
