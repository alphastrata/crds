...
def env_str_to_bool(varname, val):
    ...
    if val in ["False", "false", "FALSE", "F", "f", "0", False, 0]:
        ...
        raise ValueError(
            "Invalid value " + repr(val) + " for boolean env var " + repr(varname)
        )
    return rval
...
def env_str_to_int(varname, val):
    ...
    try:
        ...
        raise ValueError(
            "Invalid value for "
            + repr(varname)
            + " should have a decimal integer value but is "
            + repr(str(val))
        )
...
class BooleanConfigItem(ConfigItem):
    ...
    def set(self, val):
        ...
        return super(BooleanConfigItem, self).set(
            str(env_str_to_bool(self.env_var, str(val)))
        )
...
def set_crds_ref_subdir_mode(mode, observatory):
    ...
    if writable_cache_or_verbose("skipping subdir mode write."):
        ...
        hv.cache_atomic_write(
            mode_path, mode, "Couldn't save sub-directory mode config"
        )
...
def check_crds_ref_subdir_mode(mode):
    ...
    assert (
        mode in CRDS_REF_SUBDIR_MODES
    ), "Invalid CRDS cache subdirectory mode = " + repr(mode)
    return mode
...
def get_download_plugin():
    ...
    """
    if (
        "CRDS_DOWNLOAD_MODE" in os.environ
        and os.environ["CRDS_DOWNLOAD_MODE"].lower() != "plugin"
    ):
        return None
        ...
        for path in ["/usr/local/bin", "/usr/bin", "/sw/bin"]:
            ...
            if os.path.exists(program):
                return (
                    program
                    + " --no-check-certificate --quiet ${SOURCE_URL}  -O ${OUTPUT_PATH}"
                )
    return None
...
CRDS_MAPPING_URI = StrConfigItem(
    "CRDS_MAPPING_URI",
    None,
...
CRDS_REFERENCE_URI = StrConfigItem(
    "CRDS_REFERENCE_URI",
    None,
...
DOWNLOAD_CHECKSUMS = BooleanConfigItem(
    "CRDS_DOWNLOAD_CHECKSUMS",
    True,
    "Verify downloaded files match server's sha1sum.  If false,  allow bad checksums.",
)

DOWNLOAD_LENGTHS = BooleanConfigItem(
    "CRDS_DOWNLOAD_LENGTHS",
    True,
    "Verify downloaded files match server's file length.  If false,  allow bad lengths.",
)
...
CLIENT_RETRY_COUNT = IntConfigItem(
    "CRDS_CLIENT_RETRY_COUNT",
    1,
    "Integer number of times CRDS should retry download errors.  No retries == 1.",
)
...
CLIENT_RETRY_DELAY_SECONDS = IntConfigItem(
    ...
    0,
    "Seconds CRDS should wait between retries.  Defaults to 0.",
)
...
CLIENT_TIMEOUT = IntConfigItem(
    "CRDS_CLIENT_TIMEOUT_SECONDS",
    3600,
    "Seconds to wait for a CRDS network request to complete.",
)
...
def get_server_url(observatory):
    ...
    return os.environ.get(
        "CRDS_SERVER_URL", CRDS_DEFAULT_SERVERS.get(observatory, None)
    )
...
def pop_crds_uri(filepath):
    ...
    if filepath.startswith("crds://"):
        ...
        assert not os.path.dirname(
            filepath
        ), "crds:// must prefix a filename with no path."
    return filepath
...
USE_LOCKING = BooleanConfigItem(
    "CRDS_USE_LOCKING", True, "Set to False to turn off CRDS cache locking."
)
...
FILE_RE = re.compile(
    complete_re(FILE_RE_STR)
)  # at min *should not* contain % < > \ { }
FILE_PATH_RE_STR = r"([/A-Za-z0-9_\- ]{1,256})?" + FILE_RE_STR
FILE_PATH_RE = re.compile(
    complete_re(FILE_PATH_RE_STR)
)  # at min *should not* contain % < > \ { }
...
CONTEXT_DATETIME_RE_STR = (
    r"\d\d\d\d\-\d\d\-\d\d" + r"(T\d\d:\d\d:\d\d" + r"(\.\d+)?" + r")?"
)
...
def is_simple_crds_mapping(mapping):
    ...
    if isinstance(mapping, str):
        ...
        return is_valid_mapping_name(basename) and (
            locate_mapping(mapping) == locate_mapping(basename)
        )
    else:
        return False
...
def is_mapping_spec(mapping):
    ...
    True
    """
    return is_mapping(mapping) or (
        isinstance(mapping, str) and bool(CONTEXT_RE.match(mapping))
    )
...
def is_context_spec(mapping):
    ...
    False
    """
    return is_context(mapping) or (
        isinstance(mapping, str) and bool(PIPELINE_CONTEXT_RE.match(mapping))
    )
...
S3_ENABLED = BooleanConfigItem(
    "CRDS_S3_ENABLED", False, "When True, enables S3 streaming"
)

S3_RETURN_URI = BooleanConfigItem(
    "CRDS_S3_RETURN_URI",
    False,
    "When True, getreferences() returns S3 URIs instead of cache paths.",
)

OBSERVATORY = StrConfigItem(
    "CRDS_OBSERVATORY",
    None,
