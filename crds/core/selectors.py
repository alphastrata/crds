...
class Selection(tuple):
    ...
    def _cmp_key(self, key):
        return (
            tuple(str(field) for field in key)
...
class Selector:
    ...
    def __init__(
        self, parameters, selections=None, rmap_header=None, merge_selections=None
    ):
        assert isinstance(
        ...
        if "merge_overlaps" in self._rmap_header:
            self._merge_overlaps = str(self._rmap_header["merge_overlaps"]).upper() in [
                "TRUE",
                "1",
            ]
        else:
        ...
        if selections is not None:
            ...
            self._selections = [
                Selection(s) for s in self.condition_selections(selects)
            ]
        else:
            ...
            assert isinstance(
                merge_selections, list
            ), "merge_selections should be a sorted item list,  not: " + repr(
                merge_selections
            )
            # Because merge_selections are derived from selections, no substitutions are done here
            self._raw_selections = (
                merge_selections  # XXX not really,  nominally unused XXXX
            )
            self._selections = merge_selections
        self._parkey_map = self.get_parkey_map()
        self._comment_parkeys = tuple(
            name.upper() for name in self._rmap_header.get("comment_parkeys", ())
        )
    ...
    def todict_flat(self):
        ...
        for key, val in self._raw_selections:
            if isinstance(val, Selector):
                ...
                key = tuple(
                    [
                        ", ".join(glob_list(str(parval)))
                        for parval in self.fix_singleton_match_case(key)
                    ]
                )
                flat.extend(
                    [
                        self.fix_singleton_match_case(key) + row
                        for row in nested["selections"]
                    ]
                )
            else:
    ...
    def delete_match_param(self, parameter):
        ...
        for i, (old_key, choice) in enumerate(self._raw_selections):
            try:
                ...
                log.verbose(
                    "Replacing match case", repr(old_key), "with", repr(new_key)
                )
            except ValueError:
    ...
    def condition_selections(self, selections):
        ...
        """
        result = [
            (self.condition_key(key), value) for (key, value) in selections.items()
        ]
    ...
    def choose(self, header):
        ...
        for selection in self.get_selection(
            lookup_key
        ):  # iterate over weighted selections, best match first.
            try:
                log.verbose("Trying", selection, verbosity=60)
                return self.get_choice(
                    selection, header
                )  # recursively,  what's final choice?
            ...
            if isinstance(choice, Selector):
                ...
                for val in choice:
                    if not isinstance(val, str):
                        raise ValidationError(
                            "Non-string tuple value for choice "
                            + repr(choice)
                            + " at "
                            + repr(key)
                        )
                ...
                for val in choice:
                    if not isinstance(val, str):
                        raise ValidationError(
                            "Non-string dictionary key for choice "
                            + repr(choice)
                            + " at "
                            + repr(key)
                        )
                for val in choice.values():
                    if not isinstance(val, str):
                        raise ValidationError(
                            "Non-string dictionary value for choice "
                            + repr(choice)
                            + " at "
                            + repr(key)
                        )
                ...
                raise ValidationError(
                    "Illegal type for selector primitive value", repr(choice)
                )
    ...
    def _validate_value(self, name, value, valid_list, runtime=True):
        ...
        """
        if (
            value in valid_list or utils.condition_value(value) in valid_list
        ):  # typical |-glob valid_list membership
            log.verbose(
                "Value for",
                repr(name),
                "of",
        ...
        if runtime and (
            "*" in valid_list or "ANY" in valid_list or "N/A" in valid_list
        ):
            log.verbose(
                "Valid list for",
                repr(name),
                "includes wild cards. OK, no other check.",
                verbosity=60,
            )
            return
        ...
        if not valid_list:
            log.verbose(
                "Valid list for", repr(name), "is empty.  No other check.", verbosity=60
            )
            return
        ...
        if value.lower().startswith("between"):
            log.verbose(
                "Checking 'between' expression for",
                repr(name),
                "of",
                repr(value),
                verbosity=60,
            ...
            return
        if (
            len(valid_list) == 1 and ":" in valid_list[0]
        ):  # handle ranges in .tpns as n1:n2
            min, max = [
                float(x) for x in valid_list[0].split(":")
            ]  # normalize everything as float
        ...
        if name in self._substitutions and value in self._substitutions[name]:
            ...
            self._validate_value(
                name, self._substitutions[name][value], valid_list, runtime
            )
            return
        raise ValidationError(
            " parameter="
    ...
    def merge(self, other):
        raise AmbiguousMatchError(
            "More than one match was found at the same weight and "
            + self.short_name
    ...
    def _delete(self, selections, terminal):
        ...
        for i in range(len(selections) - 1, -1, -1):
            ...
            if choice == terminal:
                log.verbose(
                    "Deleting selection[%d] with key='%s' and terminal='%s'"
                    % (i, selection[0], terminal)
                )
                del selections[i]
                deleted += 1
        ...
        if len(classes) > 1:  # add or insert nested selector
            if i is None:
                ...
                if isinstance(old_value, Selector):
                    ...
                    old_value._insert(
                        header, value, parkey[1:], classes[1:], valid_values_map
                    )
                else:
                    log.verbose(
                        "Selector replaces terminal at",
                        repr(key),
                        "adding new selector.",
                    )
                    new_value = self._create_path(
                        header, value, parkey[1:], classes[1:]
                    )
                    self._replace_item(old_key, new_value)
        else:  # add or replace primitive result
            if i is None:
                log.verbose(
                    "Modify couldn't find", repr(key), "adding new value", repr(value)
                )
    ...
    def _create_path(self, header, value, parkey, classes):
        ...
        if classes:  # add new Selectors defined by classes
            selector_class = utils.get_object(
                "crds.selectors." + classes[0] + "Selector"
            )
    ...
    def _add_item(self, key, value):
        ...
        self.__init__(
            self._parameters,
            dict_wo_dups(self._raw_selections),
            rmap_header=self._rmap_header,
        )
    ...
    def difference(self, new_selector, path=(), pars=(), top_selector=True):
        ...
        def short_name(obj):
            return (
                obj.short_name if isinstance(obj, Selector) else obj.__class__.__name__
            )
        ...
        for key, choice in self._raw_selections:
            ...
            if key not in new_selector_keys:
                if isinstance(choice, Selector):
                    differences.extend(
                        choice.flat_diff(
                            "deleted {} rule for".format(self.short_name),
                            path + (pkey,),
                ...
                if isinstance(new_selector_choice, Selector):
                    differences.extend(
                        new_selector_choice.flat_diff(
                            "added {} rule for".format(self.short_name),
                            path + (pkey,),
                            pars + (self._parameters,),
                        )
                    )
                    ...
                    differences.append(
                        msg(key, "added terminal", repr(new_selector_choice))
                    )
        return differences
    ...
    def flat_diff(self, change, path, pars):
        ...
        for key, choice in self._raw_selections:
            ...
            if isinstance(choice, Selector):
                diffs.extend(
                    choice.flat_diff(change, path + (pkey,), pars + (self._parameters,))
                )
            else:
                msg = self._get_msg(path, pars)
    ...
    def match(self, value):
        ...
        if self._operator == "AND":
            return (
                1
                if (
                    (self._matcher1.match(value) == 1)
                    and (self._matcher2.match(value) == 1)
                )
                else -1
            )
            ...
            return (
                ...
                if (
                    (self._matcher1.match(value) == 1)
                    or (self._matcher2.match(value) == 1)
                )
                else -1
            )
        else:
...
class MatchSelector(Selector):
    ...
    def winnowing_match(self, header):
        ...
        for _weight, match_tuples in sorted_candidates:
            if len(match_tuples) > 1:
                ...
                subselectors = tuple(
                    [remaining[match_tuple].choice for match_tuple in match_tuples]
                )
    ...
    def _check_valid_values(self, valid_values_map):
        ...
        for name in self._parameters:
            if name not in valid_values_map:
                log.verbose_warning(
                    self.short_name, "Parameter ", repr(name), " is unchecked."
                )
    ...
    def _validate_raw_key(self, key, valid_values_map):
        ...
        for other in self.keys():
            if (
                key != other
                and match_superset(other, key)
                and not different_match_weight(key, other)
            ):
                raise AmbiguousMatchError(
...
class VersionAfterSelector(UseAfterSelector):
    ...
    def _validate_version(self, version):
        assert isinstance(version, str) and config.VERSION_RE.match(
            version
...
class BracketSelector(Selector):
    ...
    def get_choice(self, bracket_selection, header):
        ...
        result1 = super(BracketSelector, self).get_choice(
            bracket_selection.less, header
        )
        if bracket_selection.less != bracket_selection.greater:
            result2 = super(BracketSelector, self).get_choice(
                bracket_selection.greater, header
            )
        else:
            result2 = result1
...
class VersionRelation(ComparableMixin):
        ...
        if self.relation_str.replace("=", "").strip() == "default":
            ...
            if match:
                ...
                try:
                    ...
                    raise ValidationError(
                        "Invalid version expression in:", repr(self.relation_str)
                    ) from exc
            else:
                raise ValidationError(
    ...
    def _check_compatible(self, other):
        if self.version == "default":
            return True
        elif isinstance(self.version, numbers.Number) and isinstance(
            other.version, numbers.Number
        ):
            return True
    ...
    def _instantiate(self, parkeys, rmap_header, parents=None):
        ...
        for key, selpars in self.selections:
            if isinstance(selpars, Parameters):
                ...
                selections[key] = selpars._instantiate(
                    parkeys[1:], rmap_header, parents + [parent]
                )
            else:
...
def check_duplicates(items, parents=None):
    ...
    for key, value in items:
        if key in already_seen:
            log.error(
                "Duplicate entry at",
                " ".join(parents + [repr(key)]),
                "=",
                repr(value),
                "vs.",
                repr(already_seen[key]),
            )
        else:
