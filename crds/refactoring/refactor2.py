...
def rmap_delete_references(old_rmap, new_rmap, deleted_references):
    ...
    for reference in deleted_references:
        ...
        assert reference not in formatted, (
            "Rules update failure.  Deleted "
            + srepr(reference)
            + " still appears in new rmap."
        )
    return new
...
def rmap_check_modifications(old_rmap, new_rmap, old_ref, new_ref, expected=("add",)):
    ...
    with open(old_rmap) as pfile:
        old_count = len(
            [line for line in pfile.readlines() if os.path.basename(old_ref) in line]
        )
    with open(new_rmap) as pfile:
        new_count = len(
            [line for line in pfile.readlines() if os.path.basename(new_ref) in line]
        )
...
def del_rmap_header(rmapping, new_filename, header_key):
    ...
    """
    log.verbose(
        "Deleting header value in", srepr(rmapping.basename), "for", srepr(header_key)
    )
...
def get_parameter_index(parkey, parameter_name):
    ...
    raise crexc.CrdsError(
        "Can't find index for", repr(parameter_name), "in parkey", repr(parkey)
    )
...
def set_rmap_parkey(rmapping, new_filename, parkey, *args, **keys):
    ...
    references_headers = {
        refname: get_refactoring_header(rmapping.filename, refname, required_keywords)
    ...
    rmapping = rmap_insert_references_by_matches(
        new_filename, new_filename, references_headers
    )
    return rmapping
...
def get_refactoring_header(rmapping, refname, required_keywords):
    ...
    assert len(headers2) == 1, "Can't refactor file with more than one match: " + srepr(
        refname
    )
...
def set_rmap_substitution(
    rmapping, new_filename, parameter_name, old_text, new_text, *args, **keys
):
    log.info(
...
def cat_rmap(rmapping, new_filename, header_key, *args, **keys):
    ...
    if header_key is not None:
        log.info(
            ...
            srepr(header_key),
            "=",
            srepr(rmapping.header[header_key]),
        )
    else:
        log.info("-" * 80)
...
def add_rmap_useafter(rmapping, new_filename, *args, **keys):
    ...
    set_rmap_header(
        ...
        new_filename,
        "classes",
        repr(rmapping.obs_package.DEFAULT_SELECTORS),
        *args,
        **keys
    )
...
def certify_rmap(rmapping, new_filename, source_context=None, *args, **keys):
    ...
    script = certify.CertifyScript(
        "crds.certify {0} --comparison-context {1}".format(
            new_filename, rmapping.filename
        ),
        reset_log=False,
        print_status=False,
    )
...
def apply_rmap_fixers(rmapping, new_filename, fixers, *args, **keys):
    ...
    for fixer in fixers:
        ...
        rmap.replace_rmap_text(
            rmapping, new_filename, old_text, new_text, *args, **keys
        )
        rmapping = rmap.load_mapping(new_filename)
...
class RefactorScript2(cmdline.Script):
    ...
    def add_args(self):
        ...
        self.add_argument(
            ...
            type=str,
            nargs="*",
            help="Explicitly specify list of rmaps for refactoring.",
        )
        self.add_argument(
            "--header-key",
            type=str,
            default=None,
            help="Header keyword for header commands.",
        )
        self.add_argument(
            "--header-value", type=str, default=None, help="Value for header commands."
        )
        self.add_argument(
            "--parkey", type=str, default=None, help="New parkey value for set_parkey."
        )
        self.add_argument(
            "--parameter-name",
            type=str,
        ...
        self.add_argument(
            ...
            type=str,
            default=None,
            help="Source text for replace_text command.",
        )
        self.add_argument(
            "--new-text",
            type=str,
            default=None,
            help="Replacement text for replace_text command.",
        )
        self.add_argument(
            "--inplace",
            action="store_true",
    ...
    def main(self):
        ...
        if self.args.best_effort:
            ...
            config.ALLOW_SCHEMA_VIOLATIONS.set(
                True
            )  # Don't fail for data model bad value errors
        ...
        if self.args.rmaps:  # clean up dead lines from file lists
            self.args.rmaps = [
                self.resolve_context(mapping)
                for mapping in self.args.rmaps
                if mapping.strip()
            ]

        if self.args.references:
            self.args.references = [
                self.locate_file(reference) for reference in self.args.references
            ]
        ...
        with log.error_on_exception("Refactoring operation FAILED"):
            if self.args.command == "insert_reference":
                ...
                raise ValueError(
                    "Unknown refactoring command: " + repr(self.args.command)
                )
        ...
        if self.args.rmaps:
            for rmap_name in self.args.rmaps:
                with log.error_on_exception("Failed processing rmap", srepr(rmap_name)):
                    ...
                    new_filename = self._process_rmap(
                        func, rmapping=rmapping, **keywords
                    )
            ...
            instruments = (
                pmapping.selections.keys()
                if "all" in self.args.instruments
                else self.args.instruments
            )
            for instr in instruments:
                with log.augment_exception(
                    "Failed loading imap for",
                    repr(instr),
                    "from",
                ...
                types = (
                    imapping.selections.keys()
                    if "all" in self.args.types
                    else self.args.types
                )
                for filekind in types:
                    with log.error_on_exception(
                        "Failed processing rmap for", repr(filekind)
                    ):
                        # , "from",
    ...
    def _process_rmap(self, func, rmapping, *args, **keys):
        ...
        new_filename = (
            ...
            else os.path.join(".", rmapping.basename)
        )
    ...
    def _setup_source_context(self):
        ...
        if self.args.sync_files:
            errs = sync.SyncScript(
                "crds.sync --contexts {}".format(self.source_context)
            )()
    ...
    def set_header_key(self):
        ...
        self.rmap_apply(
            set_rmap_header,
            header_key=self.args.header_key,
            header_value=self.args.header_value,
        )
        ...
        for (instrument, filekind) in categorized:
            try:
                ...
                spec_file = os.path.join(
                    os.path.dirname(self.obs_pkg.__file__),
                    "specs",
