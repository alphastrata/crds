...
UpdateTuple = namedtuple(
    "UpdateTuple", ["instrument", "filekind", "old_reference", "new_reference"]
)
...
than errors as the default.
    ...
    def __init__(self, *args, **keys):
        ...
        self.process_filekinds = [
            typ.lower() for typ in self.args.types
        ]  # list of filekind str's
        ...
        self.new_headers = headers.HeaderGenerator(
            None, None, None
        )  # Abstract for pylint, replaced later

        # comparison variables
        self.compare_prior = None  # bool
        self.old_headers = headers.HeaderGenerator(
            None, None, None
        )  # Abstract for pylint, replaced later
    ...
    def complex_init(self):
        ...
        assert not (
            self.args.sync_references and self.readonly_cache
        ), "Readonly cache,  cannot fetch references."
        ...
        if self.args.diffs_only:
            assert (
                self.new_context and self.old_context
            ), "--diffs-only only works for context-to-context bestrefs."
            differ = diff.MappingDifferencer(
                self.observatory,
                self.old_context,
                self.new_context,
                include_header_diffs=True,
                hide_boring_diffs=True,
            )
        ...
        (
            self.compare_prior,
            self.old_headers,
            self.old_bestrefs_name,
        ) = self.init_comparison(datasets_since)
    ...
    def auto_datasets_since(self):
        ...
        for instrument in self.oldctx.selections:
            ...
            if deleted_references:
                deleted_exp_time = matches.get_minimum_exptime(
                    old_imap, deleted_references
                )
    ...
    def setup_contexts(self):
        ...
        if self.args.old_context is not None:
            log.verbose(
                "Using explicit old context", repr(self.args.old_context), verbosity=25
            )
    ...
    def warn_bad_context(self, name, context, instrument):
        ...
        if context is None:
            return
        bad_contained = heavy_client.get_bad_mappings_in_context(
            self.observatory, context, instrument
        )
        if bad_contained:
            ...
            log.verbose(
                name, "=", repr(context), "contains bad rules", repr(bad_contained)
            )
    ...
    def warn_bad_updates(self):
        ...
        for (dataset, updates) in sorted(self.updates.items()):
            for update in sorted(updates):
                bad_files += self.warn_bad_reference(
                    dataset, update.instrument, update.filekind, update.new_reference
                )
    ...
    def init_headers(self, context, datasets_since):
        ...
        if self.args.files:
            the_headers = headers.FileHeaderGenerator(
                context, self.files, datasets_since
            )
            ...
            self.require_server_connection()
            log.info(
                "Computing bestrefs for db datasets for", repr(list(self.instruments))
            )
            the_headers = headers.InstrumentHeaderGenerator(
                context,
                self.instruments,
                datasets_since,
                self.args.save_pickle,
                self.server_info,
            )
        ...
        if self.args.load_pickles:
            ...
            if the_headers:  # combine partial correction headers field-by-field
                ...
                the_headers.update_headers(
                    self.pickle_headers.headers, only_ids=self.only_ids
                )
            else:  # assume pickles-only sources are all complete snapshots
                log.verbose(
                    "Computing bestrefs solely from pickle files:",
                    repr(self.args.load_pickles),
                )
        ...
        if compare_prior:
            if self.args.old_context:
                ...
                if self.args.fetch_old_headers:
                    log.verbose(
                        "Fetching old headers independently allowing them to differ from new headers."
                    )
    ...
    def main(self):
        ...
        if self.complex_init():
            for i, dataset in enumerate(self.new_headers):
                if i != 0 and i % 1000 == 0:
                    log.verbose(
                        self.get_stat("datasets"), "sources processed", verbosity=5
                    )
    ...
    def process(self, dataset):
        ...
        with log.error_on_exception("Failed processing", repr(dataset)):
            ...
            if dataset in self.drop_ids:
                ...
                log.verbose(
                    "Skipping", repr(dataset), "not in --only-ids", verbosity=80
                )
                return
    ...
    def _process(self, dataset):
        ...
        if self.compare_prior:
            ...
            if self.args.old_context:
                self.active_header = (
                    old_header
                ) = self.old_headers.get_lookup_parameters(dataset)
                old_bestrefs = self.get_bestrefs(
                    instrument, dataset, self.old_context, old_header
                )
            else:
                old_bestrefs = self.old_headers.get_old_bestrefs(dataset)
            updates, kill_list = self._compare_bestrefs(
                instrument, dataset, old_bestrefs, new_bestrefs
            )
            if self.args.optimize_tables:
                updates = self.optimize_tables(dataset, updates)
        else:
            updates, kill_list = self._screen_bestrefs(
                instrument, dataset, new_bestrefs
            )
        if (
            self.args.update_pickle
        ):  # XX  mutating input bestrefs to support updated pickles
    ...
    def get_bestrefs(self, instrument, dataset, context, header):
        ...
        with log.augment_exception(
            "Failed determining reference types for",
            repr(dataset),
            "with respect to",
            ...
            if reftypes is None:
                return {}
        with log.augment_exception(
            "Failed computing bestrefs for data",
            repr(dataset),
            "with respect to",
            repr(context),
        ):
    ...
    def determine_reftypes(self, instrument, dataset, context, header):
        ...
        with log.verbose_warning_on_exception(
            "Failed determining reftypes for", repr(dataset)
        ):
    ...
    def verbose_with_prefix(self, dataset, instrument, filekind, *args, **keys):
        ...
        if log.should_output(*args, **keys):
            log.verbose(
                self.format_prefix(dataset, instrument, filekind), *args, **keys
            )
        ...
        for filekind in sorted(newrefs):
            ...
            if filekind in self.skip_filekinds:
                self.verbose_with_prefix(
                    dataset, instrument, filekind, "Skipping type.", verbosity=55
                )
                continue

            new_ok, new = self.handle_na_and_not_found(
                "New:", newrefs, dataset, instrument, filekind
            )
    ...
    def _compare_bestrefs(self, instrument, dataset, oldrefs, newrefs):
        ...
        for filekind in filekinds:
            ...
            _old_ok, old = self.handle_na_and_not_found(
                "Old:", oldrefs, dataset, instrument, filekind
            )
            ...
            if new != old:
                if self.args.differences_are_errors:  # regression mode vs. reprocessing
                    ...
                    if (
                        old != "N/A" and new != "N/A"
                    ) or self.args.na_differences_matter:
                        self.log_and_track_error(
                            dataset,
                            instrument,
                            filekind,
    ...
    def handle_na_and_not_found(self, name, bestrefs, dataset, instrument, filekind):
        ...
        if ref.startswith("NOT FOUND N/A"):
            self.verbose_with_prefix(
                dataset, instrument, filekind, "Bestref is natural N/A.", verbosity=60
            )
            ref = "N/A"
        elif ref in ("NOT FOUND NO MATCH FOUND.", "UNDEFINED", "NONE", "", "*"):
            if (
                self.args.undefined_differences_matter
            ):  # track these when N/A is being scrutinized, regardless of diff.
                self.log_and_track_error(
                    dataset,
                    instrument,
                    filekind,
    ...
    def print_affected(self):
        ...
        """
        affected_products = {
            self.dataset_to_product_id(dataset) for dataset in self.unkilled_updates
        }
        ...
        for dataset in sorted(self.updates):
            if self.updates[dataset]:
                ...
                print(
                    "{} {} {}".format(
                        dataset.lower(),
                        self.updates[dataset][0].instrument.lower(),
                        " ".join(types),
                    )
                )
        sys.stdout.flush()
        ...
        for dataset in sorted(self.updates):
            for update in self.updates[dataset]:
                print(
                    dataset.lower()
                    + " "
                    + " ".join([str(val).lower() for val in update])
                )
        sys.stdout.flush()
    ...
    def sync_references(self):
        ...
        api.dump_references(
            self.new_context, sorted(synced_references), raise_exceptions=self.args.pdb
        )
...
def assign_bestrefs(
    filepaths, context=None, reftypes=(), sync_references=False, verbosity=-1
):
