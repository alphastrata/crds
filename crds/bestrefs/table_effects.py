...
def is_reprocessing_required(
    dataset, dataset_parameters, old_context, new_context, update
):
    ...
    log.verbose(
        f"Deep Reference examination between {old_reference} and {new_reference} initiated.",
        verbosity=25,
    )

    with log.error_on_exception(
        "Failed fetching comparison reference tables:", repr([old_ref, new_ref])
    ):
    ...
    try:
        ...
        if dataset_id in deep_look.stub_input:
            log.verbose_warning(f'Substituting header for dataset "{dataset}"')
            dataset_parameters = deep_look.stub_input[dataset_id]["headers"]
            log.verbose_warning("headers = ", dataset_parameters, verbosity=25)

        log.verbose(
            deep_look.preamble, f"Dataset headers = {dataset_parameters}", verbosity=75
        )

        log.verbose(
            deep_look.preamble,
            f"Comparing references {old_reference} and {new_reference}.",
            verbosity=75,
        )
        ...
        log.verbose(
            deep_look.preamble,
            f"Reprocessing is {'' if deep_look.is_different else 'not '}required.",
            verbosity=25,
        )
        ...
        log.verbose_warning(f"Deep examination error: {error.message}", verbosity=25)
        log.verbose_warning(
            "Deep examination failed, presuming reprocessing.", verbosity=25
        )
        return True
...
def mode_equality(modes_a, modes_b):
    ...
    if len(modes_a) == len(modes_b):
        return modes_a == modes_b if len(modes_a) > 0 else True
    else:
        return False
...
class DeepLook:
    ...
    def __init__(self):
        self.stub_input = {}
        self.preamble = f"Rule {self.__class__.__name__}:"
        self.metavalues = {}
        self.mode_fields = {}
    ...
    def from_filekind(cls, instrument, filekind):
        ...
        name = f"{instrument}_{filekind}".lower()
        log.verbose(f"Instantiating rules for reference type {name}.", verbosity=25)

        if name in cls.rules:
            return cls.rules[name]()
        else:
            raise DeepLookError(
                f"No rules for instrument {instrument} and reference file kind {filekind}"
            )
    ...
    def are_different(self, headers, old_reference, new_reference):
        ...
        for field in self.mode_fields:
            constraint_values[field] = (
                str_to_number(headers_low[field]) if field in headers_low else None
            )
        ...
        for key in constraint_values:
            if key in self.metavalues:
                if constraint_values[key] in self.metavalues[key]:
                    constraint_values[key] = self.metavalues[key][
                        constraint_values[key]
                    ]

        # Read the references
        data_old = tables.tables(old_reference)[
            0
        ]  # XXXX currently limited to FITS extension 1
        ...
        log.verbose(
            self.preamble, "Old reference matching rows:\n", mode_rows_old, verbosity=75
        )
        log.verbose(
            self.preamble, "New reference matching rows:\n", mode_rows_new, verbosity=75
        )
        ...
        if self.is_different:
            self.message = (
                "Selection rules have executed and the selected rows are different."
            )
        else:
            self.message = (
                "Selection rules have executed and the selected rows are the same."
            )
