...
class TestSync(test_config.CRDSTestCase):
    ...
    def test_sync_contexts(self):
        ...
        for name in crds.get_cached_mapping("hst_cos_deadtab.rmap").reference_names():
            ...
            with open(config.locate_file(name, "hst"), "w+") as handle:
                handle.write("foo")
        self.run_script(
            "crds.sync --contexts hst_cos_deadtab.rmap --fetch-references --check-files",
            2,
        )
        self.run_script(
            "crds.sync --contexts hst_cos_deadtab.rmap --fetch-references --check-files --repair-files",
            2,
        )
        self.run_script(
            "crds.sync --contexts hst_cos_deadtab.rmap --fetch-references --check-files --repair-files"
    ...
    def test_sync_explicit_files(self):
        ...
        self.run_script(
            "crds.sync --files hst_cos_deadtab.rmap --check-files --repair-files --check-sha1sum"
        )
    ...
    def test_sync_readonly_cache(self):
        ...
        self.run_script(
            "crds.sync --contexts hst_cos_deadtab.rmap"
        )  # sync to establish config area for sure
        self.run_script(
            "crds.sync --contexts hst_cos_deadtab.rmap --fetch-references --readonly-cache"
        )
    ...
    def test_sync_organize_flat(self):
        ...
        self.run_script(
            "crds.sync --contexts hst_cos_deadtab.rmap --fetch-references --organize=flat"
        )
    ...
    def test_sync_dataset_ids(self):
        ...
        self.run_script(
            "crds.sync --contexts hst.pmap --dataset-ids LA9K03CBQ:LA9K03CBQ --fetch-references"
        )
    ...
    def test_purge_mappings(self):
        ...
        self.assertEqual(
            r.reference_names(), ["s7g1700gl_dead.fits", "s7g1700ql_dead.fits"]
        )
        self.assertEqual(
            rmap.list_references("*", "hst"),
            ["s7g1700gl_dead.fits", "s7g1700ql_dead.fits"],
        )
        self.assert_crds_exists("s7g1700gl_dead.fits")
        self.assert_crds_exists("s7g1700ql_dead.fits")
        self.run_script(
        ...
        self.assertEqual(
            rmap.list_references("*", "hst"),
            ["w3m1716tj_imp.fits", "w3m17170j_imp.fits", "w3m17171j_imp.fits"],
        )
