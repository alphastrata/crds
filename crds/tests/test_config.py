...
CRDS_TESTING_CACHE = os.environ.get(
    "CRDS_TESTING_CACHE", "no-test-cache-defined-see-TESTING"
)
...
class CRDSTestCase(unittest.TestCase):
    ...
    def setUp(self, *args, **keys):
        ...
        self.old_state = setup(
            cache=self.cache, url=self.server_url, clear_existing=self.clear_existing
        )
...
def setup(
    cache=CRDS_SHARED_GROUP_CACHE, url=None, clear_existing=True, observatory=None
):
