...
def apply_with_retries(func, *pars, **keys):
    ...
    for retry in range(retries):
        try:
            ...
            log.verbose_warning(
                "FAILED: Attempt", str(retry + 1), "of", retries, "with:", str(exc)
            )
            log.verbose_warning(
                "FAILED: Waiting for", delay, "seconds before retrying"
            )  # waits after total fail...
            time.sleep(delay)
            exc2 = exc
    raise exc2
...
class ServiceCallBinding:
    ...
    def _call_service(self, parameters, url):
        ...
        try:
            ...
            raise exceptions.ServiceError(
                "CRDS jsonrpc failure " + repr(self.__service_name) + " " + str(exc)
            ) from exc
    ...
    def classify_exception(self, decoded):
        ...
        if "Channel" in decoded and "not found" in decoded:
            ...
            msg = (
                "CRDS jsonrpc failure " + repr(self.__service_name) + " " + str(decoded)
            )
...
def crds_encode(obj):
    ...
    return dict(crds_encoded="1.0", crds_payload=ascii)
