...
def get_url(pipeline_context, filename):
    ...
    utils.deprecate("crds.client.get_url()", "2020-09-01", "crds.client.get_flex_uri()")
...
def get_total_bytes(info_map):
    ...
    try:
        return sum(
            [
                int(info_map[name]["size"])
                for name in info_map
                if "NOT FOUND" not in info_map[name]
            ]
        )
...
def get_sqlite_db(observatory):
    ...
    assert (
        not config.get_cache_readonly()
    ), "Readonly cache, updating the SQLite database cannot be done."
...
def get_best_references_by_ids(
    context, dataset_ids, reftypes=None, include_headers=False
):
    ...
    """
    try:
        bestrefs = S.get_best_references_by_ids(
            context, dataset_ids, reftypes, include_headers
        )
    except Exception as exc:
        raise CrdsLookupError(str(exc)) from exc
    return bestrefs
...
def get_best_references_by_header_map(context, header_map, reftypes=None):
    ...
    """
    try:
        bestrefs_map = S.get_best_references_by_header_map(
            context, header_map, reftypes
        )
    except Exception as exc:
        raise CrdsLookupError(str(exc)) from exc
    return bestrefs_map
...
def _get_server_info():
    ...
    try:
        ...
        raise CrdsNetworkError(
            f"Failed downloading cache config from: {config_uri}:", srepr(exc)
        ) from exc
    return info
...
def get_default_observatory():
    ...
    if obs != "none":
        return obs
    return (
        observatory_from_string(get_crds_server()) or get_server_observatory() or "jwst"
    )
...
def file_progress(
    activity, name, path, bytes, bytes_so_far, total_bytes, nth_file, total_files
):
...
class FileCacher:
    ...
    def get_local_files(self, names):
        ...
        if downloads:
            ...
            log.verbose(
                "Skipping download for cached files", sorted(names), verbosity=60
            )
            n_bytes = 0
    ...
    def download(self, name, localpath):
        ...
        assert (
            not config.get_cache_readonly()
        ), "Readonly cache,  cannot download files " + repr(name)
        try:
    ...
    def plugin_download(self, filename, localpath):
        ...
        plugin_cmd = plugin_cmd.replace(
            "${FILE_SHA1SUM}", self.info_map[filename]["sha1sum"]
        )
    ...
    def get_data_http(self, filename):
        ...
        try:
            ...
            file_size = utils.human_format_number(
                self.catalog_file_size(filename)
            ).strip()
...
def dump_mappings3(
    pipeline_context, ignore_cache=False, mappings=None, raise_exceptions=True
):
    ...
    return FileCacher(pipeline_context, ignore_cache, raise_exceptions).get_local_files(
        mappings
    )
...
def dump_references3(
    pipeline_context, baserefs=None, ignore_cache=False, raise_exceptions=True
):
...
def dump_files(
    pipeline_context=None, files=None, ignore_cache=False, raise_exceptions=True
):
    ...
    references = [
        os.path.basename(name) for name in files if not config.is_mapping(name)
    ]
    if mappings:
    ...
    return (
        dict(list(m_paths.items()) + list(r_paths.items())),
        m_downloads + r_downloads,
        m_bytes + r_bytes,
    )
...
def cache_references(pipeline_context, bestrefs, ignore_cache=False):
    ...
    if config.S3_RETURN_URI:
        ...
        localrefs = FileCacher(
            pipeline_context, ignore_cache, raise_exceptions=False
        ).get_local_files(wanted)[0]
    ...
    for filetype, refname in bestrefs.items():
        if isinstance(refname, tuple):
            ...
            if "NOT FOUND" in refname:
                ...
                log.verbose(
                    "Reference type", srepr(filetype), "defined as", srepr(refname)
                )
