...
def get_exptypes(instrument=None):
    ...
    """
    tpninfos = get_schema_tpninfos(
        config.locate_file("jwst_nirspec_area_0005.fits", "jwst")
    )
    values = set()
...
def _x_schema_to_flat(schema):
    ...
    if schema_type == "object":
        ...
        log.verbose_warning(
            "Schema item has unhandled type", repr(schema_type), verbosity=80
        )
    return results
    ...
    for key, value in flat.items():
        if key.endswith(".TYPE"):
            ...
            if datatype is not None:
                ...
                log.verbose(
                    "Adding tpn constraint from DM schema:", repr(tpn), verbosity=65
                )
                tpns.append(tpn)
            else:
