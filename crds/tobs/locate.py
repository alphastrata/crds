...
def decompose_newstyle_name(filename):
    ...
    assert instrument in INSTRUMENTS + [""], (
        "Invalid instrument " + repr(instrument) + " in name " + repr(filename)
    )
    assert filekind in FILEKINDS + [""], (
        "Invalid filekind " + repr(filekind) + " in name " + repr(filename)
    )
    assert re.fullmatch(r"\d*", serial), (
        "Invalid id field " + repr(id) + " in name " + repr(filename)
    )
...
def locate_file(refname, mode=None):
    ...
    (
        ...
        _serial,
        _ext,
    ) = get_reference_properties(refname)
