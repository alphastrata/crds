...
class SubmissionResult(
    collections.namedtuple(
        "SubmissionResult", ["error_count", "warning_count", "ready_url"]
    )
):
...
class Submission(object):
    ...
    def __init__(self, observatory, string, context=None, *args, **kwargs):
        ...
        if observatory not in ["hst", "jwst", "roman"]:
            raise ValueError(
                'Observatory "{}" must be one of "hst" or "jwst" or "roman".'
            )
        if string not in ["bit", "cit", "dev", "int", "ops", "pub", "test"]:
            raise ValueError(
                'String "{}" must be either "ops", "test", "pub", "dev", "bit", "cit", or "int".'
            )
        ...
        self._form_description = {
            field["key"]: field for field in self._form_description
        }
        ...
        self._required_keys = {
            x for x in self._form_description if self._form_description[x]["required"]
        }
        self._optional_keys = {
            x
            for x in self._form_description
            if not self._form_description[x]["required"]
        }
    ...
    def __setitem__(self, key, value):
        ...
        if ("choices" in self._form_description[key]) and (
            ...
            matches = [
                x
                for x in self._form_description[key]["choices"]
                if x.lower() == value.lower()
            ]
            if len(matches) != 1:
                raise ValueError(
                    "'{}' must be a valid choice: {{{}}}".format(
                        key, ", ".join(self._form_description[key]["choices"])
                    )
                )
    ...
    def add_file(self, filename):
        ...
        if not os.access(filename, os.R_OK):
            raise FileNotFoundError(
                "'{}' does not exist or is not readable.".format(filename)
            )
    ...
    def help(self):
        ...
        for key, field in self._form_description.items():
            ...
            if "choices" in field:
                ...
                print(
                    "  {",
                    ", ".join(["'{}'".format(x) for x in field["choices"]]),
                    "}",
                    sep="",
                )
            print()
    ...
    def validate(self):
        ...
        empty_keys = {
            key for key in self._required_keys if self[key] == ""
        }  # Don't flag False booleans
        if empty_keys:
            raise ValueError(
                "These keywords cannot be empty:\n    " + "\n    ".join(empty_keys)
            )

        # Make sure files were associated with the submission:
        if len(self.files) == 0:
            raise NoFilesSelected(
                "No files have been added to submission.  "
        ...
        if invalid_char is not None:
            raise ValueError(
                f'Invalid character detected: "{invalid_char}" - only alphanumeric, dashes, underscores, periods, commas, forward slashes and equals signs are allowed.'
            )
