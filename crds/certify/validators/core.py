...
class Validator:
    ...
    def __init__(self, info, context=None):
        ...
        if not (
            self.info.presence in generic_tpn.TpnInfo.presences
            or self.conditionally_required
        ):
        ...
        for tab in tables.tables(filename):
            if self.name in tab.colnames:
                ...
                for i, value in enumerate(
                    tab.columns[self.name]
                ):  # compare to TPN values
    ...
    def handle_missing(self, header):
        ...
        if presence in ["R", "P", True]:
            raise MissingKeywordError(
                "Missing required", self._keytype_descr, repr(self.name)
            )
        elif presence in ["W"]:
            ...
            raise TpnDefinitionError(
                "Unexpected validator 'presence' value:", repr(self.info.presence)
            )
        return "UNDEFINED"
    ...
    def conditionally_required(self):
        ...
        if has_condition:
            if not self._presence_condition_code:
                self._presence_condition_code = compile(
                    self.info.presence, repr(self.info), "eval"
                )
            return True
        else:
            return False
    ...
    def is_applicable(self, header):
        ...
        if self._presence_condition_code:
            try:
                presence = eval(
                    self._presence_condition_code, header, self._eval_namespace
                )
                log.verbose(
                    "Validator",
                    self.info,
                ...
                log.warning(
                    "Failed checking applicability of",
                    repr(self.info),
                    "skipping check : ",
                    str(exc),
                )
    ...
    def get_required_copy(self):
        ...
        idict = (
            required.info._asdict()
        )  # returns OrderedDict,  method is public despite _
        idict["presence"] = "R"
...
class PedigreeBaseValidator(KeywordValidator):
    ...
    def should_be(self):
        ...
        return (
            "One of " + repr(self._values_wo_date) + " or " + repr(self._dated_format)
        )
    ...
    def _inflight_exc(self, clarification=""):
        ...
        """
        return ValueError(
            "INFLIGHT format should be "
            + repr(self._dated_format)
            + " : "
            + clarification
        )
        ...
        with log.augment_exception("Invalid value:", repr(value)):
            ...
            if len(values) == 1:
                if pedigree not in self._values_wo_date:
                    raise ValueError(
                        "Value " + repr(pedigree) + " cannot be used as a simple value."
                    )
                else:
                    return pedigree
            elif len(values) == 3:
            ...
            try:
                ...
                try:
                    ...
                    timestamp.Jwstdate.get_datetime(
                        value
                    )  # re-execute to replace exception raised
            log.warning(
    ...
    def check_header(self, filename, header):
        ...
        try:
            ...
            raise RequiredConditionError(
                "Failed checking constraint", repr(self._expr), ":", str(exc)
            )
        if not satisfied:
            raise RequiredConditionError(
                "Constraint", str(self._expr), "is not satisfied."
            )
...
class FileexistsValidator(Validator):
    ...
    def _check_value(self, filename, value):
        ...
        if not os.path.exists(checked_path):
            raise ValueError(
                "Required CRDS file " + repr(value) + " does not exist in CRDS cache."
            )
    ...
    def condition(self, value):
        ...
        if (
            "$" in value
        ):  # remove IRAF-style path prefix from SYNPHOT TMC and TMT filename column values
            crds_name = crds_name.split("$")[-1]
        if (
            "[" in value
        ):  # split off HDU index trailer,  or SYNPHOT parameterization trailer
