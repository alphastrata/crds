...
class Certifier:
    ...
    def __init__(
        ...
        self.provenance_keys = list(
            provenance_keys
            or utils.get_observatory_package(self.observatory).PROVENANCE_KEYWORDS
        )
    ...
    def get_validators(self):
        ...
        checkers = validators.get_validators(
            self.observatory, self.filename, context=self.context
        )
    ...
    def get_corresponding_rmap(self):
        ...
        pmap = crds.get_pickled_mapping(
            self.context, ignore_checksum="warn"
        )  # reviewed
...
class ReferenceCertifier(Certifier):
    ...
    def complex_init(self):
        ...
        self.all_simple_names = [
            val.name for val in self.validators if val.info.keytype == "H"
        ]
        ...
        for checker in self.validators:
            try:
                ...
                if presence == "W":  # excludes "O"
                    log.warning(
                        "Checking", repr(checker.info.name), "failed:", str(exc)
                    )
                else:
    ...
    def load(self):
        ...
        header = abstract.ensure_keys_defined(
            header, needed_keys=[checker.complex_name for checker in self.validators]
        )
        return header
    ...
    def map_ref_keys_to_dataset_keys(self, header):
        ...
        if rmapping:
            with self.error_on_exception(
                "Error mapping reference names and values to dataset names and values"
            ):
                header = rmapping.locate.reference_keys_to_dataset_keys(
                    rmapping, header
                )
        return header
    ...
    def array_validators(self):
        ...
        return [
            checker for checker in self.validators if checker.info.keytype in ["A", "D"]
        ]
    ...
    def add_array_keywords(self, header):
        ...
        for checker in self.array_validators:
            ...
            if (checker.complex_name not in header) or (
                checker.info.keytype == "D"
                and header[checker.complex_name]["DATA"] is None
            ):
            ...
            if is_undefined:
                ...
                if checker.complex_name not in seen:
                    try:
                        ...
                        self.log_and_track_error(
                            "Missing required array", repr(checker.name)
                        )
    ...
    def get_mode_column_names(self):
        ...
        mode_columns = []
        with self.error_on_exception(
            "Error finding unique row keys for", repr(self.basename)
        ):
            instrument, filekind = utils.get_file_properties(
                self.observatory, self.filename
            )
            ...
            if mode_columns:
                if tables.ntables(self.filename):
                    ...
                    log.verbose(
                        "No tables defined in reference.   Skipping row checks."
                    )
            else:
    ...
    def certify_reference_modes(self):
        ...
        for i in range(0, min(n_new_segments, n_old_segments)):
            with self.error_on_exception(
                "Checking tables modes in segment", i, "of", repr(self.filename)
            ):
    ...
    def find_old_reference(self, context, reffile):
        ...
        log.verbose(
            "Resolving comparison reference for",
            repr(reffile),
            "in context",
            repr(context),
        )
        with log.warn_on_exception(
            "Failed resolving comparison reference for table checks"
        ):
    ...
    def _find_old_reference(self, context, reffile):
        ...
        for diff_tup in diffs:
            if diff.diff_action(diff_tup) == "replace":
                ...
                assert (
                    dummy == refname
                ), "Bad replacement inserting '{}' into '{}'".format(
                    reffile, reference_mapping.name
                )
                break
        else:
            log.info(
                "No file corresponding to",
                repr(reffile),
                "in context",
                repr(reference_mapping.name),
            )
            return None
        ...
        try:
            match_files = api.dump_references(
                reference_mapping.name, baserefs=[match_refname], ignore_cache=False
            )
            match_file = match_files[match_refname]
            if not os.path.exists(
                match_file
            ):  # For server-less mode in debug environments w/o Central Store
                raise IOError(
            ...
            log.info(
                ...
                repr(refname),
                "against",
                repr(os.path.basename(match_file)),
            )
        except Exception as exc:
            log.warning(
                "Failed to obtain reference comparison file",
                repr(match_refname),
                ":",
                str(exc),
            )
    ...
    def check_table_modes(self, old_table, new_table):
        ...
        log.verbose(
            "Checking tables modes of '{}' against comparison reference '{}'".format(
                new_reference_ex, old_reference_ex
            )
        )
        old_modes, old_all_cols = table_mode_dictionary(
            "old reference", old_table, self.mode_columns
        )
        if not old_modes:
            log.info(
                "No modes defined in comparison reference",
        ...
        new_modes, new_all_cols = table_mode_dictionary(
            "new reference", new_table, self.mode_columns
        )
        if not new_modes:
            log.info(
                "No modes defined in new reference",
        ...
        for mode in sorted(old_modes):
            ...
            diffs = self.compare_row_values(
                mode, old_modes[mode][1], new_modes[mode][1]
            )
            if not diffs:
                log.verbose(
                    "Mode",
    ...
    def check_asdf_tag(self):
        ...
        if self.observatory.lower() == "roman":
            ...
            with asdf.open(self.filename, _force_raw_types=True) as f:
                ...
                if not uri_match(
                    "asdf://stsci.edu/datamodels/roman/tags/reference_files/{}-*".format(
                        f["roman"]["meta"]["reftype"].lower()
                    ),
                    f["roman"]._tag,
                ):
                    log.error(
...
def find_governing_rmap(context, reference):
    ...
    assert instrument == g_instrument, (
        "Comparison context inconsistent with reference file: "
        + repr(instrument)
        + " vs. "
        + repr(g_instrument)
    )
    assert filekind == g_filekind, (
        "Comparison context inconsistent with reference type: "
        + repr(filekind)
        + " vs. "
        + repr(g_filekind)
    )
    log.verbose(
        "Reference '{}' corresponds to rmap '{}' in context '{}'".format(
...
def table_mode_dictionary(generic_name, tab, mode_keys):
    ...
    log.info(
        "Mode columns defined by spec for",
        generic_name,
        basename,
        "are:",
        repr(mode_keys),
    ...
    log.info(
        "Checking for duplicate modes using intersection",
        sorted(list(set(mode_keys) & set(all_cols))),
    )
    ...
    for i, row in enumerate(tab.rows):
        ...
        if not mode:
            log.info(
                "Empty actual mode in",
                generic_name,
                basename,
                "with candidate mode columns",
                mode_keys,
            )
            return {}, []
...
class FitsCertifier(ReferenceCertifier):
    ...
    def __init__(self, *args, **keys):
        ...
        if self.run_fitsverify:
            ...
            if status == 0:
                ...
                log.warning(
                    "External fitsverify program is enabled but not found on PATH."
                )
    ...
    def load(self):
        ...
        if not self.filename.endswith(".fits"):
            ...
            return
        with data_file.fits_open_trapped(
            self.filename, checksum=bool(config.FITS_VERIFY_CHECKSUM)
        ) as pfile:
            pfile.verify(option="exception")  # validates all keywords
            if "ASDF" in pfile:
                self.log_and_track_error(
                    "FITS files must not include an ASDF extension"
                )
                    ...
                    for card in hdu.header.cards:
                        if card.keyword == key:
                            if self.interesting_value(card.value):
                                log.info(
                                    "[" + str(i) + "]", key, card.value, card.comment
                                )
                            if key in unseen:
                                unseen.remove(key)
...
def interpret_fitsverify_output(status, output):
    ...
    if log.warnings() - warnings:
        log.warning(
            "Fitsverify output contains errors or warnings CRDS recategorizes as WARNINGs."
        )
...
class UnknownCertifier(Certifier):
    ...
    def certify(self):
        ...
        with log.augment_exception(
            "Error parsing ", exception_class=InvalidFormatError
        ):
            self.load()
    ...
    def certify(self):
        ...
        with asdf.open(self.filename) as af:
            if not self._VERSION_RE.match(af["asdf_library"]["version"]):
                log.warning(
                    "File written with dev version of asdf library:",
                    af["asdf_library"]["version"],
                )
        ...
        if derived_from is not None:
            if derived_from.name == self.basename:
                log.verbose(
                    "Mapping",
                    repr(self.filename),
                    "did not change relative to context",
                    repr(self.context),
                )
            else:
...
def certify_file(
    ...
    with trap(filename, "Certifier instantiation error"):
        ...
        observatory = (
            utils.file_to_observatory(filename) if observatory is None else observatory
        )
...
def certify_files(
    ...
    if (
        check_rmap
    ):  # Requires checking all files in parallel, hence not in certify_file()
        if not skip_banner:
            banner()
...
def check_rmap_updates(observatory, context, filepaths):
    ...
    if not references:
        return
    observatory = (
        utils.file_to_observatory(references[0]) if observatory is None else observatory
    )
    organized = utils.organize_files(
        observatory, references
    )  # { (instrument, filekind) : [references,...] }
...
For more information on the checks being performed,  use --verbose or --verbosity=N where N > 50.
    ...
    def main(self):
        ...
        if self.args.comparison_context in ["none", "NONE", "None"]:
            log.warning(
                "It is no longer possible to run the certifier without a comparison context.  Using default context instead."
            )
            self.args.comparison_context = None

        assert (self.args.comparison_context is None) or config.is_mapping_spec(
            self.args.comparison_context
        ), (
            "Specified --context file "
            + repr(self.args.comparison_context)
            + " is not a CRDS mapping."
        )
        assert (self.args.comparison_reference is None) or not config.is_mapping_spec(
            self.args.comparison_reference
        ), (
            "Specified --comparison-reference file "
            + repr(self.args.comparison_reference)
            + " is not a reference."
        )
        ...
        if self.args.comparison_reference:
            comparison_reference = config.locate_reference(
                self.args.comparison_reference, self.observatory
            )
        else:
            comparison_reference = None
        ...
        certify_files(
            ...
            comparison_reference=comparison_reference,
            compare_old_reference=self.args.comparison_context
            or self.args.comparison_reference,
    ...
    def log_and_track_error(self, filename, *args, **keys):
        ...
        super(CertifyScript, self).log_and_track_error(
            filename, instrument, filekind, *args, **keys
        )
    ...
    def mapping_closure(self, files):
        ...
        for file_ in files:
            ...
            if config.is_mapping(file_):
                with self.error_on_exception(
                    file_, "Problem loading submappings of", repr(file_)
                ):
                    mapping = crds.get_cached_mapping(file_, ignore_checksum="warn")
                    more_files = {
                        config.locate_mapping(name) for name in mapping.mapping_names()
                    }
                    more_files = (
                        more_files - {config.locate_mapping(mapping.basename)}
                    ) | {file_}
