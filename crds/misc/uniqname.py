...
--dry-run can be used to demo renaming by printing what the new name would be.
    ...
    def add_args(self):
        ...
        self.add_argument(
            "-o", "--output-path", help="Output renamed files to this directory path."
        )
        self.add_argument(
            "-b", "--brief", action="store_true", help="Produce less output."
        )
        self.add_argument(
            "--fits-errors",
            action="store_true",
    ...
    def main(self):
        ...
        for filename in self.files:
            assert config.is_reference(filename), (
                "File "
                + repr(filename)
                + " does not appear to be a reference file.  Only references can be renamed."
            )
    ...
    def rewrite(self, filename, uniqname):
        ...
        with data_file.fits_open(
            ...
            try:
                ...
                hdus.writeto(
                    uniqname, output_verify=verify_mode, checksum=self.args.add_checksum
                )
...
def has_checksum(filename):
    ...
    """
    with data_file.fits_open(
        filename, mode="readonly", do_not_scale_image_data=True
    ) as hdus:
        for hdu in hdus:
...
def uniqname(old_path):
    ...
    new_name = UniqnameScript(
        "crds.misc.uniqname --files {0} --standard --remove-original --fits-errors {1}".format(
